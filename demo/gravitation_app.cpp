...
class GravityPhysicsSystem {
 public:
  ...
  void update(std::vector<VdemGameObject>& objs, float dt, unsigned int substeps = 1) {
  ...
  glm::vec2 computeForce(VdemGameObject& fromObj, VdemGameObject& toObj) const {
 ...
 private:
  void stepSimulation(std::vector<VdemGameObject>& physicsObjs, float dt) {
...
class Vec2FieldSystem {
 public:
  void update(
      ...
      std::vector<VdemGameObject>& physicsObjs,
      std::vector<VdemGameObject>& vectorField) {
...
std::unique_ptr<VdemModel> createSquareModel(VdemDevice& device, glm::vec2 offset) {
...
std::unique_ptr<VdemModel> createCircleModel(VdemDevice& device, unsigned int numSides) {
  std::vector<VdemModel::Vertex> uniqueVertices{};
...
void App::run() {
  // create some models
  std::shared_ptr<VdemModel> squareModel = createSquareModel(
      device,
      {.5f, .0f});  // offset model by .5 so rotation occurs at edge rather than center of square
  std::shared_ptr<VdemModel> circleModel = createCircleModel(device, 64);

  // create physics objects
  std::vector<VdemGameObject> physicsObjects{};
  auto red = VdemGameObject::createGameObject();
  ...
  auto blue = VdemGameObject::createGameObject();
  ...
  for (int i = 0; i < gridCount; i++) {
    for (int j = 0; j < gridCount; j++) {
      auto vf = VdemGameObject::createGameObject();
